...
COPY dependencies/instantclient* /temp/
...
COPY src/ .
RUN ls -lrt
RUN for i in logs framework/sessions framework/views framework/cache ; do mkdir -p $APP_HOME/storage/$i; done
RUN chmod g+s $APP_HOME/storage
RUN chown -R www-data:www-data $APP_HOME
RUN php -d memory_limit=-1 `which composer` dump-autoload --quiet --optimize
RUN if [ -f $APP_HOME/artisan ]; then for i in cache:clear view:clear route:cache config:cache; do php $APP_HOME/artisan $i; done; fi
